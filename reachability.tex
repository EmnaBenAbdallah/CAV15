...
\annotMF{Peut-être écrire : $\texttt{A}_\texttt{I}$ et $\texttt{B}_\texttt{J}$} \repEB{c'est fait}
...
\annotMF{Peut-être écrire : $\texttt{B}_\texttt{J}$ et $\texttt{B}_\texttt{K}$} \repEB{c'est fait}
\modMF{%
...
\end{lstlisting}
\annotMF{N'est-il pas possible de réunir ces deux règles avec des bornes 1--1 de la façon suivante : 1 \{activeFromTo(B,J,K,T)\} 1 $\leftarrow$ ... ? Sinon, voir à peut-être enlever la cardinalité pour une règle plus simple à comprendre.} \repEB{ En fait non ce n'est pas possible vu que la première est une règle normale et la deuxième est une contrainte.D'ailleurs si je mets comme tu me demande je n'aurai pas le meme résultat. En effet la première règle donne l'ensemble de tous les changements possibles pour un instant T  en plus il faut que ça soit un ensemble pour trouver tous les chemins. Je peux les enlever pour la clarté mais pour le resultat des deux scripts n'est pas le même}
...
\modifEB{Else the answer will be eliminated by the constraint \ref{c2-a} which verify if all processes of the goal are satisfied.}
\todo{Il manque la contrainte qui élimine les mauvais parcours !} \repEB{Je les ai pas mises car ils s'agit de 3 lignes et encore plus d'éxplication et je ne vois pas si c'est important de les mettre car il faudrait les expliquer aussi}
\begin{lstlisting}
...
if the shortest path to the goal requires $K$ steps with $K > N$. \modifEB{It also may lengthen the resolution if the shortest path requires $n$ steps with $n << N$}
\annotMF{Est-ce que ça ne risque pas aussi de rallonger anormalement la résolution si le plus court chemin est en $n$ étapes avec $n << N$ ?} \repEB{Si ça risque...}
One solution is to use an incremental computation mode,
which is especially tackled by the incremental solver \textsc{iclingo}~\cite{gebser2008user}. \modifEB{In fact \textsc{iclingo} further includes the following guidelines:}
...
\annotMF{Peut-on citer le manuel d'utilisation de Clingo comme référence ?} \repEB{Effectivement la réference \cite{gebser2008user} est un manuel d'utilisation pour Clingo ainsi que IClingo}
...
Regarding the part of local steps, \modifEB{that is, all rules, facts, and integrity constraints computed in one step are dismissed before the next incremental step},
