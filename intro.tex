\todo{à revoir} \\
For more than a decade, model-checking and SAT-solvers developments have been strongly connected \cite{biere1999symbolic}. Among different reasons, the main issue is related to the different problems with BDD-based symbolic approaches, which are:
\begin{itemize}
\item BDD-based structures require some canonical form to be defined so that a total order can be defined; 
...
Firstly, we show that starting from one PH model, it is possible to find all possible stable states (fix points which is a specific attractor with only one state \cite{wuensche1998genomic}).
