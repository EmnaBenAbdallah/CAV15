...
to solve the problem of reachability that consists of deciding if,
starting from a given initial state, it is possible to reach a given full local state or a partial local state. In fact this dynamic property of reachability is considred also a CTL property of model checking EF and AF having a condition on the lenght of the path.%
Besides we aim at finding the fixed points, that are, the stable states in which it is not possible
any more to have any evolution of the model.%
...
We show that our approach succeeds in processing large models with a considerable number of components and interactions.
}
