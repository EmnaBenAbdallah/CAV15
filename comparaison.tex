...
\annotMF{Il y a une confusion entre méthodes et outils ici je pense.} \repEB{J'ai modifié}
\annotMF{Il faut trouver un autre nom pour “Rocca's method”.} \repEB{c'est vrai ; "transition state networks with ASP?}
...
\todo{Rajouter le nombre de sortes dans le tableau ou la description} \repEB{avec ou sans les sortes coopératives? car pour PH y'en a plus de composants que pour les autres modèles}
\annotMF{Il faut expliciter ce qu'on entend par “state” et “process”. Il fudrait peut-être aussi changer les termes ; je propose : “full state” et “partial state”.} \modifEB{Je préfère ta proposition mais tu vois ça prend plus d'espace et le tableau dépasse les limites }

\todo{Il n'y a pas de résultats sur le calcul des poitns fixes dans le tableau} \repEB{En fait pour la méthode de Rocca le script qu'il m'ont envoyé génére quelques erreurs lors de la compilation. pour libddd y'avait pas l'option (--fixpoint) ne génère rien comme résultat et GINsim répond très rapidement mais il faudrait calculer tout le graphe de transition avant. }
...
\todo{Seulement la partie accessible ou l'ensemble du graphe ?} \repEB{Je pense que oui mais je n'arrive pas à le m'assurer car il m'a fallut les états initiaux pour tester l'atteignabilité à ces états stables}
...
%\end{lstlisting}
%%
\annotMF{Cette ligne de commande ne me semble pas nécessaire. Mais si on la garde, il faut en donner un autre exemple plus tôt, sans l'argument -{}-imax=n.} \modifEB{ok t'as raison je l'ai enlevée et remplacée par juste l'option \texttt{"--imax=n"}}
