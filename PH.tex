...
\todo{Glu pour la définition \ref{def:NextState}} \repEB{ça veut dire quoi Glu?}
...
\annotMF{“Stable state” ? “Fix point” ? “Fixed point” ?} \repEB{Je pense garder fix point}
\end{definition}
...
\label{def:reachability}
 ...
 If $s \in \PHl$ is a state and $A \subseteq \Proc$ \repEB{ with $\Proc$ is a set of processes that we find at most one process of each sort $\{a_i, b_j,..., z_k\}$ }
...
\annotMF{Il serait peut-être mieux de parler de “reachability property” plutôt que “question”.}  \repEB{ Je l'ai changé} 
\end{definition}
